...
$(".parallax-confirmar").parallax({
  imageSrc: "resources/portada.jpg",
});
...
// Copiar texto (funciona aunque Fancybox clone el contenido)
(function setupCopyDelegation() {
  ...
  document.addEventListener("click", async (e) => {
    ...
    // Buscar el texto dentro del MISMO popup/clon donde está el botón
    const scope = btn.closest(".fancybox-content") || document;
...
(function setupWhatsAppButton() {
  const phone = "5493415591308"; // <-- tu número
  const mensaje = "Hola! Tengo una consulta sobre el casamiento 😊";
...
(function setupCopyDelegation() {
  ...
  document.addEventListener("click", async (e) => {
    ...
    // Importante: buscar el texto dentro del MISMO popup/clon donde está el botón
    const scope = btn.closest(".fancybox-content") || document;
    const el = scope.querySelector(selector) || document.querySelector(selector);
    if (!el) return;
